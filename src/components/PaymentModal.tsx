...
const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose, onComplete, totalAmount,onSend }) => {
  ...
  const handlePayment = (method: 'card' | 'cash') => {
    if (method === 'card' || (method === 'cash' && parseFloat(cashAmount) >= totalAmount)) {
      onSend(method);
      onComplete();
