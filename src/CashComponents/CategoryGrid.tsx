    ...
    <div className="p-2 bg-gray-200">
      ...
      <div className="grid grid-cols-3 gap-2">
        {categories.map((category) => (
          <div 
            key={category.id}
            className={`aspect-w-1 aspect-h-1 cursor-pointer ${activeCategory === category.id ? 'ring-2 ring-blue-500' : ''}`}
            onClick={() => setActiveCategory(category.id)}
          >
            <div className="w-full h-full bg-white flex flex-col items-center justify-center p-4">
              <div className="flex-grow flex items-center justify-center text-5xl font-light text-gray-300">
                {category.code}
              </div>
              <div className="mt-2 text-center text-sm">{category.name}</div>
            </div>
          </div>
        ))}
      </div>
